...
from huggingface_client.hfClient import huggingface_infer, huggingface_infer_1, huggingface_infer_2
...
async def extract_data(object_id: str):
    ...
    headers = {
        ...
        "Authorization": "Bearer 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",
    ...
    prompt = f"""
            You are a document verification system having the permission to extract data from all kinds of documents
                ...
                Here are example outputs {json.dumps(exampleOutputs.output_example1)} {json.dumps(exampleOutputs.output_example2)} {json.dumps(exampleOutputs.output_example3)} {json.dumps(exampleOutputs.output_example4)} {json.dumps(exampleOutputs.output_example5)} {json.dumps(exampleOutputs.output_example6)}
        """
    response = huggingface_infer_1(filepath=file_path, prompt=prompt)
...
def cheque_signature_compare_llama4(object_id_1: str, object_id_2: str):
    ...
    response = huggingface_infer_2(prompt=prompt, filepath1=image1_path, filepath2=image2_path)
    return response

@mcp.tool()
def verify_sharecert_seal_llama4(object_id_1: str):
    """Verify whether medallion gaurantee seal present in provided document"""
    image1_path = attachments_folder + "/" + get_attachment_name(object_id_1)
    api_key = "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"
    invoke_url = "https://integrate.api.nvidia.com/v1/chat/completions"
    model='meta/llama-4-maverick-17b-128e-instruct'
    ...
    if len(image1_b64) < 180_000:
        print("Image is too large, please use assets API for larger images")
    assert len(image1_b64) < 180_000, \
        "To upload larger images, use the assets API (see docs)"
    ...
    image2_path="/home/ujjwal-ltim/ujjwal/Document_Digitizer/MedallionSignatureGuarantee.jpg"
    prompt = """Provided an input image and another image for reference to medallion guarantee
                Identify if the input image contains medallion guarantee or not
                Respond yes if medallion guarantee is present otherwise no 
                Respond with Yes or No, Don't provide anything else in the output
                """
    response = huggingface_infer_2(prompt=prompt, filepath1=image1_path, filepath2=image2_path)
    return response
